...
(function() {
  var CoffeeScript, commonjsEval, defineFunctionString, doctest, escodegen, esprima, f$, fetch, fs, functionEval, log, noop, pathlib, repr, rewrite, run, substring, validators, _,
    __slice = [].slice;
  ...
  f$ = function(comments) {
    return _.reduce(comments, function(_arg, comment) {
      var accum, state;
      accum = _arg[0], state = _arg[1];
      return _.reduce(_.initial(comment.value.match(/(?!\s).*/g)), function(_arg1, line) {
        var accum, initial, input_, lines, loc, location, prefix, state, _i, _ref;
        accum = _arg1[0], state = _arg1[1];
        switch (state) {
          case 0:
            if (prefix = _.first(/^>/.exec(line))) {
              return [
                __slice.call(accum).concat([{
                    loc: comment.loc,
                    input_: {
                      location: comment.loc.start,
            ...
            break;
          case 1:
            initial = 2 <= accum.length ? __slice.call(accum, 0, _i = accum.length - 1) : (_i = 0, []), (_ref = accum[_i++], loc = _ref.loc, input_ = _ref.input_);
            if (prefix = _.first(/^>/.exec(line))) {
              return [
                __slice.call(accum).concat([{
                    loc: comment.loc,
              ...
              location = input_.location, lines = input_.lines;
              return [
                __slice.call(initial).concat([{
                    loc: {
                      start: loc.start,
                      end: comment.loc.end
                    },
                    input_: {
                      location: location,
                      lines: __slice.call(lines).concat([line.substr(prefix.length)])
                    }
                  }]), 1
              ];
            } else {
  ...
  rewrite.js = function(input) {
    var codeChunks, comments, f, processComment, tests;
    f = function(expr) {
      return "function() {\n  return " + expr + "\n}";
    };
    ...
    comments = __slice.call(esprima.parse(input, {
        comment: true,
        loc: true
      }).comments).concat([{
        value: '',
    ...
    codeChunks = _.first(_.reduce(__slice.call(tests).concat([{
        loc: {
          start: {
            line: Infinity,
            column: Infinity
      ...
      return [__slice.call(chunks).concat([substring(input, start, test.loc.start)]), test.loc.end];
    }, [
      [], {
        line: 1,
        column: 0
    ...
    return _.flatten(_.zip(codeChunks, __slice.call(_.map(tests, function(test) {
        var lines;
        lines = [];
