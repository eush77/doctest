...
(function() {
  var CoffeeScript, commonjsEval, defineFunctionString, doctest, escodegen, esprima, fetch, fs, functionEval, log, noop, pathlib, repr, rewrite, run, substring, transformComments, validators, wrap, _;
        ...
        if (prefix === '>') {
          return [
            1, accum.concat({
              input: {
                loc: loc,
                lines: [line.slice(prefix.length)]
              }
            })
          ];
        } else if (state === 0) {
          ...
          return [
            1, _.initial(accum).concat({
              input: {
                ...
                lines: _.last(accum).input.lines.concat(line.slice(prefix.length))
              }
            })
          ];
        } else {
