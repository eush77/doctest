...
(function() {
  var CoffeeScript, commonjsEval, defineFunctionString, doctest, escodegen, esprima, fetch, fs, functionEval, log, noop, pathlib, replace, repr, rewrite, run, validators, _;
  ...
  replace = function(input, replacement, _arg) {
    var end, start;
    ...
    return input.split('\n').map(function(line, idx) {
      ...
      lineNumber = idx + 1;
      switch (false) {
        case (lineNumber !== (_ref = start.line) || _ref !== end.line):
          return "" + (line.substr(0, start.column)) + replacement + (line.substr(end.column + 1));
        case lineNumber !== start.line:
          return "" + (line.substr(0, start.column)) + replacement;
        case lineNumber !== end.line:
          return line.substr(end.column + 1);
        case !((start.line < lineNumber && lineNumber < end.line)):
          return '';
        default:
          return line;
      }
  ...
  rewrite.js = function(input) {
    ...
    return _.reduce(esprima.parse(input, options).comments, function(input, comment) {
      return replace(input, processComment(comment), esprima.parse(input, options).comments[0].loc);
    }, input);
  };
