...
(function() {
  ...
  fetch = function(path) {
    var abspath, fs, pathlib, script, _ref;
    ...
    if (typeof window !== 'undefined') {
      ...
      abspath = pathlib.resolve((_ref = process.env.DOCTEST_CWD) != null ? _ref : process.cwd(), path);
      return fs.readFile(abspath, 'utf8', function(err, text) {
        var ext, filepath, name, source;
        if (err != null) {
          throw err;
        }
        name = pathlib.basename(abspath);
        source = (function() {
          switch (ext = pathlib.extname(name)) {
            case '.coffee':
              return CoffeeScript.compile(rewrite(text, 'coffee'));
            case '.js':
              return rewrite(text, 'js');
            default:
  ...
  rewrite.js = function(input) {
    ...
    if (typeof window !== 'undefined') {
      ...
      return "var __doctest = require('" + __filename + "');\n" + input;
    }
  };
  ...
  rewrite.coffee = function(input) {
    ...
    lines = [];
    lines.push(typeof window !== 'undefined' ? 'window.__doctest = doctest' : "__doctest = require '" + __filename + "'");
    expr = '';
    _ref = input.split('\n');
