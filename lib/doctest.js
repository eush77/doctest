...
(function() {
  var CoffeeScript, commonjsEval, defineFunctionString, doctest, escodegen, esprima, fetch, fs, functionEval, log, noop, pathlib, repr, rewrite, run, substring, validators, _,
    __slice = [].slice;
  ...
  rewrite.js = function(input) {
    ...
    return _.chain(comments).reduce(function(_arg, _arg1) {
      var chunks, loc, start;
      chunks = _arg[0], start = _arg[1];
      loc = _arg1.loc;
      return [__slice.call(chunks).concat([substring(input, start, loc.start)]), loc.end];
    }, [
      [], {
        line: 1,
        column: 0
      }
    ]).first().zip(_.map(comments, processComment)).flatten().value().join('');
  };
