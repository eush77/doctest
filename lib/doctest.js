...
(function() {
  ...
  rewrite.js = function(input) {
    var LINES, comments, f, options, processComment, xxx;
    f = function(expr) {
      return "function() {\n  return " + expr + "\n}";
    };
    ...
    comments = esprima.parse(input, options).comments;
    LINES = input.split('\n');
    xxx = _.reduce(comments, function(_arg, comment) {
      var chunks, lineNumber, lines__, position;
      chunks = _arg[0], position = _arg[1];
      lineNumber = position.line;
      if (comment.loc.start.line === lineNumber) {
        return [__slice.call(chunks).concat([LINES[lineNumber - 1].substr(position.column, comment.loc.start.column)]), comment.loc.end];
      } else {
        ...
        return [__slice.call(chunks).concat([lines__.join('\n')]), comment.loc.end];
      }
    }, [
      [], {
        line: 1,
    ...
    return _.initial(_.flatten(_.zip(xxx, _.map(comments, processComment)))).join('');
  };
