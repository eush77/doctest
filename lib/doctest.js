...
(function() {
  ...
  rewrite.coffee = function(input) {
    var source, wrap;
    wrap = {
      input: function(test, indent) {
        return "" + indent + "__doctest.input ->\n" + indent + "  " + test.input.value + "\n" + indent;
      },
      output: function(test, indent) {
        return "" + indent + "__doctest.output " + test.output.loc.start.line + ", ->\n" + indent + "  " + test.output.value + "\n" + indent;
      }
    };
    source = _.chain(input.split('\n')).reduce(function(_arg, line, idx) {
      ...
      if (match = /^([ \t]*)#(?!##)[ \t]*(>|[.]*)(.*)$/.exec(line)) {
        ...
        if (prefix === '>') {
          return [
            value, expr ? lines.concat(wrap.input({
              input: {
                value: expr
              }
            }, indent)) : lines
