...
(function() {
  ...
  f$ = function(comments) {
    return _.last(_.reduce(comments, function(_arg, comment) {
      var accum, state;
      state = _arg[0], accum = _arg[1];
      return _.reduce(_.initial(comment.value.match(/(?!\s).*/g)), function(_arg1, line) {
        var accum, initial, input, prefix, state, _i, _ref;
        ...
        switch (state) {
          case 0:
            ...
            break;
          case 1:
            initial = 2 <= accum.length ? __slice.call(accum, 0, _i = accum.length - 1) : (_i = 0, []), (_ref = accum[_i++], input = _ref.input);
            if (prefix = _.first(/^>/.exec(line))) {
              return [
                1, accum.concat({
                  input: {
              ...
              return [
                1, initial.concat({
                  input: {
                    ...
                    lines: __slice.call(input.lines).concat([line.substr(prefix.length)])
                  }
                })
              ];
            } else {
  ...
  rewrite.js = function(input) {
    var capture, comments, f, generate, inf, tests;
    f = function(expr) {
      return "function() {\n  return " + expr + "\n}";
    };
    capture = {
      input: function(test) {
        return "__doctest.input(" + (f(test.input.lines.join('\n'))) + ")";
      },
      output: function(test) {
        return "__doctest.output(" + test.output.loc.start.line + ", " + (f(test.output.lines.join('\n'))) + ")";
      }
    };
    ...
    generate = _.partial(escodegen.generate, _, {
      indent: '  '
    });
    ...
    ]).first().zip(_.map(tests, _.compose(generate, esprima.parse, function(test) {
      var p;
      return ((function() {
